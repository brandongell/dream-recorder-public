...
if [ ! -f "$SCRIPT_DIR/.env" ]; then
    ...
    echo -ne "Enter your GOOGLE_AI_API_KEY (from Google AI Studio): "
    read -r GOOGLE_AI_API_KEY
    if [[ ! "$GOOGLE_AI_API_KEY" =~ ^[A-Za-z0-9_-]{20,}$ ]]; then
        log_error "GOOGLE_AI_API_KEY must be at least 20 alphanumeric characters. Exiting."
        exit 1
    fi
    ...
    sed -e "s|OPENAI_API_KEY=your-openai-api-key-here|OPENAI_API_KEY=$OPENAI_API_KEY|" \
        -e "s|GOOGLE_AI_API_KEY=your-google-ai-api-key-here|GOOGLE_AI_API_KEY=$GOOGLE_AI_API_KEY|" \
...
# Note: Google AI API key validation script would need to be created
# if docker compose exec dream_recorder python scripts/test_google_ai_key.py; then
#     log_info "Google AI API key is valid."
# else
#     log_warn "Google AI API key is invalid. Please check your .env file."
# fi
